...
$app->post('/register', function ($request, $response) use($database, $app) {
...
$app->post('/login', function ($request, $response, $args) use($database) {
	$username = $request->getParam('username');
	$password = $request->getParam('password');
	$user_id;
...
$app->post('/getData', function ($request, $response, $args) use($database) {
